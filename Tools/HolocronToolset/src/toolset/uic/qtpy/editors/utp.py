...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.currentHpSpin = QtWidgets.QSpinBox(parent=self.frame_4)
        ...
        self.formLayout_3.setWidget(0, QtWidgets.QFormLayout.ItemRole.FieldRole, self.currentHpSpin)
...
"          in the GFF."))
        ...
        self.hasInventoryCheckbox.setToolTip(_translate("MainWindow", "Has Inventory (GFF: HasInventory). BYTE 0/1. Engine reads via ReadFieldBYTE in LoadPlaceable. When set (1), the placeable has a container inventory; the game loads ItemList when the player opens it. When unset (0), the placeable has no inventory and ItemList is ignored. Modders: enable for chests, crates, corpses; use Edit Inventory to add items (UTI ResRefs). Empty list is valid."))
...
"               items."))
        ...
        self.nameEdit.setToolTip(_translate("MainWindow", "Name (GFF: LocName). CExoLocString. Localized name shown when the player examines or uses the placeable; the engine reads it in LoadPlaceable. Used in tooltips and dialogue. Empty is valid. Modders: set a short, clear name (e.g. \"Crate\", \"Computer\"); use the language selector to add translations. Does not affect scripts—use Tag for that."))
        ...
        self.tagEdit.setToolTip(_translate("MainWindow", "Tag (GFF: Tag). CExoString. Unique identifier for this placeable. The engine reads it via ReadFieldCExoString in LoadPlaceable. Scripts and dialogues use GetObjectByTag to find this placeable (e.g. to open it, lock it, or run scripts). Modders: keep unique per area; duplicate tags can cause the wrong object to be targeted. Empty is valid but the placeable cannot be found by tag. Match this to any script or dialogue references."))
...
"               state. In engine files often 0. Modders: leave 0 in templates. Valid range 0–255."))
        ...
        self.currentHpSpin.setToolTip(_translate("MainWindow", "Current HP (GFF: CurrentHP). SHORT. Placeable\'s current hit points when loaded; when it reaches 0 the placeable is destroyed and OnDeath runs. Engine reads via ReadFieldSHORT in LoadPlaceable. Modders: valid range 0–32767; set equal to Max HP for full strength. Plot or Min1HP can prevent destruction; Static placeables may ignore damage. Clamped to Max HP when loading from GIT in some cases."))
        self.label_26.setText(_translate("MainWindow", "Max HP:"))
        self.maxHpSpin.setToolTip(_translate("MainWindow", "Max HP (GFF: HP). SHORT. Maximum hit points; the placeable is destroyed when Current HP reaches 0 (unless Plot or Min1HP). Engine reads via ReadFieldSHORT in LoadPlaceable. Modders: valid range 0–32767; typical 10–50 for breakable placeables. Use 0 or 1 for indestructible if combined with Plot. Healing and damage update Current HP at runtime."))
        self.label_20.setText(_translate("MainWindow", "Hardness:"))
        self.hardnessSpin.setToolTip(_translate("MainWindow", "Hardness (GFF: Hardness). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadPlaceable. Damage reduction applied when the placeable is hit; each attack\'s damage is reduced by this amount. Does not prevent destruction when HP reaches 0. Modders: typical 0–10; high values make the placeable very resistant to damage until HP is depleted."))
        self.label_21.setText(_translate("MainWindow", "Fortitude:"))
        self.fortitudeSpin.setToolTip(_translate("MainWindow", "Fortitude (GFF: Fort). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadPlaceable. Fortitude save value used in some save checks (e.g. spells, effects). Modders: often 0 in vanilla files; increase for placeables that must resist certain effects. Used with Reflex and Will for full save calculations."))
        self.label_24.setText(_translate("MainWindow", "Reflex:"))
        self.reflexSpin.setToolTip(_translate("MainWindow", "Reflex (GFF: Ref). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadPlaceable. Reflex save value for the placeable; used in some save checks. Modders: often 0 in vanilla; set higher for placeables that should resist reflex-based effects (e.g. area spells)."))
        self.label_25.setText(_translate("MainWindow", "Will:"))
        self.willSpin.setToolTip(_translate("MainWindow", "Will (GFF: Will). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadPlaceable. Will save value; used in some save checks (e.g. mind-affecting or will-based effects). Modders: often 0 in vanilla; increase for placeables that should resist will-based effects."))
...
"               consumed when used to unlock. Modders: enable for one-time keys."))
        ...
        self.keyEdit.setToolTip(_translate("MainWindow", "Key Tag (GFF: KeyName). CExoString. Engine reads via ReadFieldCExoString in LoadPlaceable. Tag of the key item required to unlock when Key Required is set. The player must have an item in inventory whose Tag matches this string. Leave blank if no key is required. Modders: use the same Tag as the key UTI\'s Tag field. Case-sensitive in some engines."))
...
"                 set for locking to apply."))
        ...
        self.openLockSpin.setToolTip(_translate("MainWindow", "Open Lock DC (GFF: OpenLockDC). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadPlaceable. Difficulty class for the Security check to pick the lock. Modders: typical 10–25; 0 = trivial/unlocked. Key Required overrides this when the player uses the matching key. Lockable must be set for the lock to apply."))
