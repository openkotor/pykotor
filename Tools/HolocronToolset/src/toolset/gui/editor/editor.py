...
from toolset.gui.widgets.installation_toolbar import (
    InstallationToolbar,
    StandaloneWindowMixin,
    open_manage_installations_dialog,
)
...
class Editor(QMainWindow, StandaloneWindowMixin):
    ...
    def __init__(
        ...
        # Unified editor toolbar: InstallationToolbar (same widget as standalone windows).
    ...
    def _populate_editor_toolbar(self, toolbar: QToolBar) -> None:
        """Override in subclasses to add editor-specific controls after the installation toolbar.
    ...
    def _open_installations_manage_dialog(self) -> None:
        """Open shared Manage Installations dialog and reload toolbar on save."""
        open_manage_installations_dialog(
            self,
            on_save=self._installation_toolbar.reload_installations,
        )

    def _on_installation_changed(self, installation: HTInstallation | None) -> None:
        """Override in subclasses to react to installation changes."""
        return
