...
def _entrypoint_from_scripts(project_table: dict[str, Any]) -> tuple[str | None, list[str]]:
...
def _discover_entrypoint(src_dir: Path) -> str | None:
...
def discover_tools(repo_root: Path | None = None) -> list[ToolMetadata]:
    ...
    for tool_root in sorted(path for path in tools_dir.iterdir() if path.is_dir()):
        ...
        module_name = str(entrypoint).removesuffix(".py").replace("/", ".")
...
def resolve_tool(selector: str, repo_root: Path | None = None) -> ToolMetadata:
