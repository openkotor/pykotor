...
- `RIM` (0x20000000): Resource-image path in the resource manager (e.g. texture packs; naming per MacOS symbols / `AddResourceImageFile()`, see [PyKotor#47](https://github.com/OldRepublicDevs/PyKotor/issues/47); compare on-disk [RIM](RIM-File-Format) capsules)
- `ERF` (0x40000000): Encapsulated archives on disk ([ERF File Format](ERF-File-Format), [RIM File Format](RIM-File-Format); often `modules/*.rim`, `modules/*.mod`, `modules/*.erf`)
...
- [2DA-File-Format](2DA-File-Format) documents table layout; pair with [KEY-File-Format](KEY-File-Format), [BIF-File-Format](BIF-File-Format), and [ERF-File-Format](ERF-File-Format) for container-level behaviour.
...
- [BWM-File-Format](BWM-File-Format) — **Format specification** (binary layout, header, vertices, faces, AABB, adjacency, edges, perimeters). This section covers engine-side behavior only; the BWM wiki is the canonical format reference.

**Analysis based on:** *`vendor/swkotor.c`* and *`vendor/swkotor.h`* (Ghidra decompilation of *`swkotor.exe`* / *`swkotor2.exe`*). Merged from the former standalone BWM/AABB wiki page on 2026-03-24.

## Using agdec for further analysis

To extend these findings or verify behavior against a specific binary:

1. **Open a game binary in Ghidra:** Load `swkotor.exe` or `swkotor2.exe` (or a variant such as `k1_win_gog_swkotor.exe`, `k2_win_gog_aspyr_swkotor2.exe`) into a Ghidra project. Ensure the program is **loaded and analyzed** (e.g. Auto Analysis complete); agdec tools require an open program to query.
...
- [BWM-File-Format](BWM-File-Format) -- BWM binary layout (canonical format); [Indoor Map Builder Implementation Guide](Indoor-Map-Builder-Implementation-Guide), [Kit-Structure-Documentation](Kit-Structure-Documentation) -- Walkmesh extraction
- [KEY-File-Format](KEY-File-Format), [BIF-File-Format](BIF-File-Format), [ERF-File-Format](ERF-File-Format) -- Containers and resolution with KEY/BIF
