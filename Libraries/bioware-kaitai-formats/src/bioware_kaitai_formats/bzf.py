...
class Bzf(KaitaiStruct):
    ...
    def _read(self):
        ...
        if not self.file_type == "BZF ":
            raise kaitaistruct.ValidationNotEqualError("BZF ", self.file_type, self._io, "/seq/0")
        self.version = (self._io.read_bytes(4)).decode("ASCII")
        if not self.version == "V1.0":
            raise kaitaistruct.ValidationNotEqualError("V1.0", self.version, self._io, "/seq/1")
