...
class Ncs(KaitaiStruct):
    ...
    def _read(self):
        ...
        if not self.file_type == "NCS ":
            raise kaitaistruct.ValidationNotEqualError("NCS ", self.file_type, self._io, "/seq/0")
        ...
        if not self.size_marker == 66:
            raise kaitaistruct.ValidationNotEqualError(66, self.size_marker, self._io, "/seq/2")
