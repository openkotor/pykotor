...
class Gff(KaitaiStruct):
    ...
    class FieldEntry(KaitaiStruct):
        ...
        def field_data_offset_value(self):
            ...
            if self.is_complex_type:
                ...
                self._m_field_data_offset_value = (
                    self._root.header.field_data_offset + self.data_or_offset
                )
        ...
        def is_complex_type(self):
            ...
            self._m_is_complex_type = (
                (self.field_type == Gff.GffFieldType.uint64)
                or (self.field_type == Gff.GffFieldType.int64)
                or (self.field_type == Gff.GffFieldType.double)
                or (self.field_type == Gff.GffFieldType.string)
                or (self.field_type == Gff.GffFieldType.resref)
                or (self.field_type == Gff.GffFieldType.localized_string)
                or (self.field_type == Gff.GffFieldType.binary)
                or (self.field_type == Gff.GffFieldType.vector4)
                or (self.field_type == Gff.GffFieldType.vector3)
            )
        ...
        def is_simple_type(self):
            ...
            self._m_is_simple_type = (
                (self.field_type == Gff.GffFieldType.uint8)
                or (self.field_type == Gff.GffFieldType.int8)
                or (self.field_type == Gff.GffFieldType.uint16)
                or (self.field_type == Gff.GffFieldType.int16)
                or (self.field_type == Gff.GffFieldType.uint32)
                or (self.field_type == Gff.GffFieldType.int32)
                or (self.field_type == Gff.GffFieldType.single)
            )
        ...
        def list_indices_offset_value(self):
            ...
            if self.is_list_type:
                ...
                self._m_list_indices_offset_value = (
                    self._root.header.list_indices_offset + self.data_or_offset
                )
    ...
    class LabelEntryTerminated(KaitaiStruct):
        ...
        def _read(self):
            self.name = (KaitaiStream.bytes_terminate(self._io.read_bytes(16), 0, False)).decode(
                "ASCII"
            )
    ...
    class ResolvedField(KaitaiStruct):
            ...
            if self.entry.field_type == Gff.GffFieldType.list:
                ...
                for i in range(self.list_entry.num_struct_indices):
                    self._m_list_structs.append(
                        Gff.ResolvedStruct(
                            self.list_entry.struct_indices[i], self._io, self, self._root
                        )
                    )
        ...
        def value_localized_string(self):
            ...
            if self.entry.field_type == Gff.GffFieldType.localized_string:
                ...
                _pos = self._io.pos()
                self._io.seek(self._root.header.field_data_offset + self.entry.data_or_offset)
                self._m_value_localized_string = bioware_common.BiowareCommon.BiowareLocstring(
                    self._io
                )
                self._io.seek(_pos)
            ...
            if self.entry.field_count > 1:
                ...
                for i in range(self.entry.field_count):
                    self._m_fields.append(
                        Gff.ResolvedField(self.field_indices[i], self._io, self, self._root)
                    )
