"""TPC/TGA batch conversion using PyKotor read/write (Holocron-equivalent I/O).

Converts ``.tpc`` to ``.tga`` and ``.tga`` to ``.tpc`` via
``read_tpc`` / ``write_tpc`` (same stack as HolocronToolset texture tools).
"""
...
from pykotor.toolset.base import ToolsetBatchOutcome

PathLike = Union[str, os.PathLike[str]]


@dataclass
class TextureBatchOutcome(ToolsetBatchOutcome):
    """Outcome of :func:`batch_convert_textures`."""
