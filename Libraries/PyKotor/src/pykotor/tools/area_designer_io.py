"""Kotor.NET AreaDesigner `AreaSerializer_V0_1` JSON (`format: \"0.1\"`) load/save.

Mirrors `Kotor.DevelopmentKit.AreaDesigner.relocate.AreaSerialization.AreaSerializer_V0_1`.
Round-trip is used for interchange with .NET tools; PyKotor `.indoor` maps may embed the same
payload under `IndoorMap.area_designer_v01` for lossless studio workflow.
"""
...
def load_area_designer_v01(path: Path | str) -> AreaDesignerFileV01:
    """Load an Area Designer JSON file from disk."""
    p = Path(path)
    ...
    fmt = raw.get("format")
    if fmt != FORMAT_ID:
        msg = f"Expected format '{FORMAT_ID}', got {fmt!r}"
        raise ValueError(msg)
