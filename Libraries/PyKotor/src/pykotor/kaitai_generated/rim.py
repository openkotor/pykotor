...
class Rim(KaitaiStruct):
    ...
    class ResourceEntryTable(KaitaiStruct):
        def __init__(self, _io: KaitaiStream, _parent: Optional[Rim] = None, _root: Optional[Rim] = None):
            super(Rim.ResourceEntryTable, self).__init__(_io)
            self._parent: Rim = _parent
            self._root: Rim = _root
            self._read()
    ...
    class RimHeader(KaitaiStruct):
        ...
        def _read(self):
            ...
            if not self.file_type == "RIM ":
                raise kaitaistruct.ValidationNotEqualError("RIM ", self.file_type, self._io, "/types/rim_header/seq/0")
            self.file_version = (self._io.read_bytes(4)).decode("ASCII")
            if not self.file_version == "V1.0":
                raise kaitaistruct.ValidationNotEqualError("V1.0", self.file_version, self._io, "/types/rim_header/seq/1")
