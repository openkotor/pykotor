...
class IFO:
    ...
    Retail module load reads the ``Mod_*`` IFO fields with the defaults implemented in
    ``construct_ifo`` / ``dismantle_ifo``. Function-name and virtual-address cross-references
    are migrated to ``wiki/reverse_engineering_findings.md``. Third-party line URLs that were
    attached to ``__init__`` field comments are archived in
    ``wiki/reverse_engineering_findings_generics_ifo_init_url_comments_pre_scrub.md``.
    """
...
def construct_ifo(
    ...
    # Mod_Area_list: first entry's Area_Name, blank when absent.
